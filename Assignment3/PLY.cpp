...
void PLYObject::draw()
{
  ...
  {
	    // Temporary variables to be used when computing vertex colours
	  ...
	  {	
		  ...
		  float M[16];
		  Matrix4f mV;//The modelView matrix
		  glGetFloatv(GL_MODELVIEW_MATRIX, M);
		  for (int i = 0; i < 3; i++)
		  {
			  mV[i][3] = 0.0;mV[3][i] = 0.0;
			  for (int j = 0; j < 3; j++){mV[i][j] = M[i*4+j];}
		  }
		  mV[3][3] = 1.0;

		  //L is the light position
		  for(int i=0;i<3;i++){L[i]=light_pos[i];}
		  //V is the unit vector towards the eye.
		  multVector(V, mV, vertices[v]);
		  //H is the half-vector between L and V.The direction of the light
		  sub(H, L, V);
		  
		  //distance to the light source
		  //i.e distance between the light's position and the vertex
		  float d = normalize(H);

		  // Get parameters from GL_LIGHT0 for the ambient light ,the models ambient  light , diffuse and specular parameters
		  Vector4f light_ambient={1.0f, 1.0f, 1.0f, 0.0f },lightModel_ambient={1.0f, 1.0f, 1.0f, 0.0f },
			  light_diffuse={1.0f, 1.0f, 1.0f, 0.0f },light_specular={1.0f, 1.0f, 1.0f, 0.0f };
		  Vector4f emission = {0,0,0,0};
		  ...
		  //constantAttenuation as k1,linearAttenuation k2,quadraticAttenuation as k3
		  ...
		  //attenuation is 1/(k1+k2d+k3d^2)
		  float attenuation = 1/(k1+(k2*d)+(k3*(d*d)));
		  
		  // Start of Phong Equation
		  multVectors(dif, lightModel_ambient, ambient);
		  multVectors(amb, light_ambient, ambient);
		  scale(amb, attenuation);
		  //total = (lightModel_ambient * model_Am) + (light_ambient * model_Am)*attenuation;
		  add(total, dif, amb);
		  
		  //N is the normal vector
		  Vector3f N;
		  for(int i=0;i<3;i++){N[i]=normals[v][i];}
		  //L is the unit vector between the light source and eye
		  Vector3f L;
		  for(int i=0;i<3;i++){L[i]=H[i];}
		  
		  //Lambert's cosine law or cosine emission law
		  float diffuseCoefficent = dotProd(N,L);
		  //if N.L > 0 then the specular term is 0.
		  //so adjust by the proportion it is off by
		  if (diffuseCoefficent > 0.0)
		  {
			// diffuse light component
			  multVectors(dif,light_diffuse,diffuse);
			  scale(dif,diffuseCoefficent);
			  add(total, total, dif);
			  
			  Vector3f E;
			  for(int i=0,l=normalize(viewer_pos);i<3;i++){E[i]=viewer_pos[i];}
			  
			  //Lambert reflection component
			  scale(dif,diffuseCoefficent*2,N);
			  sub(spe, dif, L);

			   // specular light component
			  float dotProduct = dotProd(spe,viewer_pos);
			  float specular_factor;
			  if(dotProduct > 0.0){specular_factor = pow(dotProduct, shininess[0]);}
			  else {specular_factor = 0;}//pow of 0 by anything is 0
			  
			  multVectors(dif, light_specular, specular);
			  scale(dif, specular_factor*attenuation, dif);
			  add(total, total, dif);
		  }
		  //found this explanation for the equation at http://www.cpp-home.com/tutorials/211_1.htm
